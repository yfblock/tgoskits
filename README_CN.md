...
| 你的目标 | 建议先看 | 最短命令 |
| --- | --- | --- |
| 第一次跑起来 | [docs/docs/quickstart/qemu/aarch64.md](docs/docs/quickstart/qemu/aarch64.md) | `cargo xtask arceos qemu --package ax-helloworld --arch riscv64` |
| 完整开发示例 | [docs/docs/demo.md](docs/docs/demo.md) | 从零新增一个组件或者修改组件的完整开发流程示例 |
| 组件开发说明 | [docs/docs/design/architecture/arch.md](docs/docs/design/architecture/arch.md) | 从 `components/` 或 `os/arceos/modules/` 开始 |
| 开发 ArceOS | [docs/docs/arceos-guide.md](docs/docs/arceos-guide.md) | `cargo xtask arceos qemu --package ax-helloworld --arch riscv64` |
| 开发 StarryOS | [docs/docs/starryos-guide.md](docs/docs/starryos-guide.md) | `cargo xtask starry qemu --arch riscv64` |
| 开发 Axvisor | [docs/docs/manual/deploy/qemu.md](docs/docs/manual/deploy/qemu.md) | `cargo xtask axvisor test qemu --target aarch64` |
| 理解命令构建和测试矩阵 | [docs/docs/design/build/flow.md](docs/docs/design/build/flow.md) | `cargo xtask test` |
| 理解仓库如何组织众多独立组件 | [docs/docs/repo.md](docs/docs/repo.md) | `python3 scripts/repo/repo.py list` |
...
与组件仓库之间如需同步，请由维护者显式执行 `scripts/repo/repo.py pull/push`。详见 [docs/docs/repo.md](docs/docs/repo.md)。
...
Axvisor 不能只靠 `build/qemu` 两条命令直接跑起来，因为运行前仍需要 Guest 镜像、VM 配置和 rootfs。推荐先用 `os/axvisor/scripts/setup_qemu.sh` 自动准备这些运行时资源，再执行 `cargo xtask axvisor qemu --arch <arch>`。完整说明见 [docs/docs/manual/deploy/qemu.md](docs/docs/manual/deploy/qemu.md) 和 [docs/docs/axvisor-guide.md](docs/docs/axvisor-guide.md)。
...
修改组件后的验证策略见 [docs/docs/design/test/test.md](docs/docs/design/test/test.md) 和 [docs/docs/components.md](docs/docs/components.md)。
