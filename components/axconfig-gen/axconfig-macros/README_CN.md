...
`ax-config-macros` 提供了 Procedural macros for converting TOML format configurations to Rust constant definitions。它是 TGOSKits 组件集合的一部分，可用于集成 ArceOS、AxVisor 及相关底层系统软件的 Rust 项目。


> ax-config-macros 派生自 https://github.com/arceos-org/axconfig-gen
...
fn main() {
    // 在这里将 `ax-config-macros` 集成到你的项目中。
}
```
...
在线文档：[docs.rs/ax-config-macros](https://docs.rs/ax-config-macros)
