...
`ax-driver-virtio` 提供了 Wrappers of some devices in the `virtio-drivers` crate, that implement traits in the `ax-driver-base` series crates。它是 TGOSKits 组件集合的一部分，可用于集成 ArceOS、AxVisor 及相关底层系统软件的 Rust 项目。


> ax-driver-virtio 派生自 https://github.com/arceos-org/axdriver_crates
...
fn main() {
    // 在这里将 `ax-driver-virtio` 集成到你的项目中。
}
```
...
在线文档：[docs.rs/ax-driver-virtio](https://docs.rs/ax-driver-virtio)
