...
`ax-plat-aarch64-peripherals` 提供了 ARM64 common peripheral drivers with `axplat` compatibility。它是 TGOSKits 组件集合的一部分，可用于集成 ArceOS、AxVisor 及相关底层系统软件的 Rust 项目。


> ax-plat-aarch64-peripherals 派生自 https://github.com/arceos-org/axplat_crates
...
fn main() {
    // 在这里将 `ax-plat-aarch64-peripherals` 集成到你的项目中。
}
```
...
在线文档：[docs.rs/ax-plat-aarch64-peripherals](https://docs.rs/ax-plat-aarch64-peripherals)
