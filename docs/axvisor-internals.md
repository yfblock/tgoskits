...
flowchart LR
    ...
    arceosBase["ArceosBase: ax-std ax-hal axalloc ax-task ax-sync"]
...
| 层次 | 目录 | 职责 |
| --- | --- | --- |
| 宿主运行时层 | `ax-std`、`ax-hal`、`axalloc`、`ax-task` | 提供宿主机上的调度、内存、时间、控制台与硬件抽象 |
...
而 AxVisor 本体在 `src/hal/mod.rs` 中提供实现，将 `axalloc`、`ax-hal` 等真实宿主能力接进去。
