...
### 2.2 关键 API 与真实使用位置
- `LazyInit::new()`：大量平台和模块静态对象都以它声明，如 `axtask` 的运行队列、`ax-mm` 的内核地址空间、`ax-ipi` 的 IPI 队列。
...
### 3.2 关键直接消费者
- ArceOS 模块：`axtask`、`ax-mm`、`ax-ipi`、`ax-fs`、`ax-net`、`ax-display`、`ax-input` 等。
...
### 5.3 集成测试重点
- 平台设备初始化顺序。
- `axtask`、`ax-mm`、`ax-net` 等全局对象的首次构造与后续访问。
